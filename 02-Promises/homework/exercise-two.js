...
function problemA () {
  ...
  const promOne = promisifiedReadFile('poem-two/stanza-01.txt').then(st1 => blue(st1))
  const promTwo = promisifiedReadFile('poem-two/stanza-02.txt').then(st2 => blue(st2))
...
function problemB () {
  ...
  // ???
   ...
   const promises = filenames.map(f => promisifiedReadFile(f).then(prom => blue(prom)))
